 ...
 * this.roller = new YunnoriRoller(isTest);
 ...
 * 
 * // playTurn will return true if this team wins during its turn (including
 * extra
 * // turns)
 ...
 * }
 * 
 * // playTurn now returns boolean: true if the team wins during this turn (or
 * its
 * // extra turns)
 * private boolean playTurn(Team team) {
 * boolean extraTurnEarned = false;
 * boolean caughtOpponentThisTurn = false; // Track catches across all rolls in
 * this turn phase
 * 
 * // Phase 1: Rolling
 ...
 * boolean canReorder = earnedRolls.stream().anyMatch(r -> r == YunnoriRoll.YUT
 * || r == YunnoriRoll.MO);
 ...
 * System.out.println(
 * "\nRolls to process: " +
 * earnedRolls.stream().map(Enum::name).collect(Collectors.joining(", ")));
 * } else {
 * System.out.println("\nNo rolls earned this turn.");
 * }
 * 
 * // Phase 3: Processing Rolls - Only if there are rolls to process
 * boolean teamBecameWinner = false; // Flag to indicate if the team won
 * *during* processing rolls
 ...
 * caughtOpponentThisTurn = processOutcome; // Use the boolean returned by
 * processRolls
 * }
 * }
 * 
 ...
 * if (teamBecameWinner) {
 * // If the team became winner, stop any potential extra turns and signal win
 * back
 * // up the call stack.
 ...
 * // IMPORTANT: Return the result of the recursive call. This propagates the
 * win
 * // flag up.
 ...
 * System.out.println("\nProcessing roll " + (i + 1) + "/" + rolls.size() + ": "
 * + currentRoll);
 * 
 * List<Piece> playablePieces = team.getPlayablePieces();
 * 
 * if (playablePieces.isEmpty()) {
 * System.out.println(team + " has no pieces left to move for roll " +
 * currentRoll + ".");
 * continue;
 * }
 * 
 ...
 * System.out.println(team + " has no piece that can make a valid move with " +
 * currentRoll + " ("
 ...
 * int targetPosition = board.calculateTargetPosition(chosenPiece,
 * currentRoll.getSteps());
 * 
 * chosenPiece.moveTo(targetPosition);
 * 
 * // Announce the move result
 * if (oldPosition != chosenPiece.getCurrentPositionIndex() ||
 * chosenPiece.isFinished()) {
 * System.out.println(team + " " + chosenPiece.toString() + " moved from " +
 * oldPosition + ".");
 * 
 * // Check for catches at the *final* target position
 * if (chosenPiece.getCurrentPositionIndex() > 0 &&
 * chosenPiece.getCurrentPositionIndex() < 30) {
 * List<Piece> caughtOpponentPieces =
 * board.findOpponentPiecesAt(chosenPiece.getCurrentPositionIndex(),
 * team, teams);
 ...
 * while (!validInput) {
 * System.out.
 * print("Enter the desired order separated by spaces (e.g., Do Yut Gae): ");
 ...
 * System.out.println("Invalid number of rolls entered. Expected " +
 * originalRolls.size() + ".");
 ...
 * "' was not in your original rolls or was already used.");
 * currentOrderValid = false;
 * break;
 * }
 * reordered.add(roll);
 ...
 * System.out.
 * println("Error in roll matching. Some rolls from the original list were not used."
 * );
 * currentOrderValid = false;
 * }
 * }
